# Biblioteca Técnica 📚
Biblioteca Digital de Livros Técnicos

## Python

* A Byte of Python [Swaroop Chitlur].pdf
* Automate the Boring Stuff with Python [Al Sweigart, 2015].pdf
* Data Science from Scratch - First Principles with Python [Joel Grus, 2015].pdf
* Data Structures and Algorithms in Python [Goodrich, Tamassia, Goldwasser, 2013].pdf
* Deep Learning with Python - A Hands-on Introduction [Nikhil Ketkar, 2017].pdf
* Deep Learning with Python [Chollet, 2018].pdf
* Hands-On Machine Learning with Scikit-Learn and TensorFlow [Aurélien Géron, 2017].pdf
* How to Tango with Django [L. Azzopardi, D. Maxwell, 2018].pdf
* Introducing Python - Modern Computing in Simple Packages [Bill Lubanovic, 2015].pdf
* Lean Python - Learn Just Enough Python to Build Useful Tools [Paul Gerrard, 2016].pdf
* Learning Python - 3th Edition [Mark Lutz, 2008].pdf
* Learning Python, 5th Edition [Mark Lutz, 2013].pdf
* Lear Python The Hard Way [Zed A. Shaw, 2014].pdf
* Natural Language Processing with Python [S. Bird, E. Klein, and E. Loper, 2009].pdf
* Programming Computer Vision with Python [Jan Erik Solem, 2012].pdf
* Python All in One For Dummies [John Shovic and Alan Simpson, 2019].pdf
* Python Crash Course [Eric Matthes, 2016].pdf
* Python for Data Analysis - 2nd-Edition [William McKinney, 2018].pdf
* Python Machine Learning [Sebastian Raschka, 2015].pdf
* Python Pocket Reference, 5th edition [Mark Lutz, 2014].pdf
* Think Python, 2nd Edition [Allen Downey, 2016].pdf
* Web Scrapping with Python [Ryan Mitchell, 2015].pdf

## ElasticSearch

* Beginning Elastic Stack [Vishal Sharma, 2016].pdf
* Elasticsearch 5.x Cookbook - Third Edition [Alberto Paro, 2017].pdf
* Elasticsearch - A Complete Guide [B. Dixit, R. Kuć, M. Rogoziński, S. Chhajed, 2017].pdf
* Elasticsearch Blueprints [Vineeth Mohan, 2015].pdf
* ElasticSearch Cookbook - Second Edition [Alberto Paro, 2015].pdf
* ElasticSearch in Action [Gheorghe, Hinman, Russo, 2016].pdf
* ElasticSearch Indexing [Hüseyin Akdoğan, 2015].pdf
* ElasticSearch Server - Second Edition [Rafał Kuć, Marek Rogoziński, 2014].pdf
* Elasticsearch - The Definitive Guide [Clinton Gormley, Zachary Tong, 2015].pdf
* I Heart Logs [Jay Kreps, 2015].pdf
* Learning ELK Stack [Saurabh Chhajed, 2015].pdf
* Learning Kibana 5.0 [Bahaaldine Azarmi, 2017].pdf
* Mastering ElasticSearch [Rafał Kuć, Marek Rogoziński, 2013].pdf
* Mastering Elasticsearch - Second Edition [Rafał Kuć, Marek Rogoziński, 2015].pdf
* Mastering Elastic Stack [R. K. Gupta, Y. Gupta, 2017].pdf
* Monitoring Elasticsearch [Dan Noble, 2016].pdf
* Practical ELK Stack [Gurpreet S. Sachdeva, 2017].pdf
* Relevant Search [D. Turnbull, J. Berryman, 2016].pdf
* The Logstash Book [James Turnbull, 2016].pdf

## HTTP:

* HTTP Essentials [Stephen Thomas, 2001].pdf
* HTTP - The Definitive Guide [D. Gourley, B. Totty, 2002].pdf
* Learning HTTP2 [Stephen Ludin, Javier Garza, 2017].pdf

## Nginx:

* Complete NGINX Cookbook [Derek DeJonghe, 2019].pdf
* Nginx Essentials [Valery Kholodkov, 2015].pdf
* Nginx - From Beginner to Pro [Rahul Soni, 2016].pdf
